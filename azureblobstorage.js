...
module.exports = function (RED) {
    ...
    function updateBlob(file, blobService, containerName, blobName, callback) {
	node.log('Updating Blob');
        blobService.createAppendBlobFromLocalFile(containerName, blobName, file, function (error) {
            if (error) {
                node.log(error);
            }
            else {
                node.log("Blob '" + blobName + "' updated");
                node.send("Blob '" + blobName + "' updated in container '" + containerName +"'");
                callback();
            }
        });
    }
    ...
    // Main function called by Node-RED Append    
    function AzureBlobStorageAppend(config) {
        // Store node for further use
        node = this;
        nodeConfig = config;
        ...
        this.on('input', function (msg) {
            ...
            createContainer(clientContainerName, blobService, function() {
                ...
                updateBlob(msg.payload, blobService, clientContainerName, clientBlobName, function () {
    ...
    // Registration of the node into Node-RED to append
    RED.nodes.registerType("Append Blob", AzureBlobStorageAppend, {
        credentials: {
            accountname: { type: "text" },
            key: { type: "text" },
            container: { type: "text" },
        ...
        defaults: {
            name: { value: "Append in Blob Storage" },
        }
    });
